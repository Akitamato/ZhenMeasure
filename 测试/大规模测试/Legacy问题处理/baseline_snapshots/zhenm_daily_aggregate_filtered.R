...
ZhenM_standard_to_daily_filtered <- function(standard_records) {
  ...
  has_legacy_weight <- "median_weight_per_day" %in% names(dt)
  ...
  if (has_legacy_weight) {
      dt[is_outlier_wt == TRUE, median_weight_per_day := NA_real_]
  }
  ...
  result <- dt[
    ...
    .(
      ...
      # Weight: Prioritize pre-calculated daily weight metrics; recalculate only if missing
...
.apply_feed_lmm_correction <- function(daily_dt, raw_dt) {
  ...
  # 9 error flags for single record anomalies
  ...
  # Mark "normal" records in raw_dt (i.e., all 9 error flags are FALSE)
  ...
  for (flg in err_flags) {
    if (flg %in% names(raw_dt)) {
      flg_agg <- raw_dt[, .(flg_occurred = any(get(flg) == TRUE, na.rm = TRUE)), by = .(animal_id, record_date)]
      ...
      # Default to FALSE if a flag is missing in the input
      daily_features[, paste0("has_", flg) := FALSE]
    }
  }
  ...
  # If the daily normal feed intake is outside the 0 - 6 kg (6000g) range, set it to NA
  dt[, normal_feed_sum := ifelse(normal_feed_sum > 0 & normal_feed_sum <= 6000, normal_feed_sum, NA_real_)]
      ...
      if (!is.null(lmm_fit)) {
        ...
        for (i in seq_along(active_flags)) {
          ...
          if (flg_name %in% names(fixed_eff)) {
            ...
            dt[, correction_g := correction_g + abs(beta_val) * get(flg_name)]
          }
        }
