# ⚠️ 已过时（issue #5 重写）：第 94 行起内联复刻的是 V1.1.4 及更早的 LMM 口径
#（响应 normal_feed_sum 且按 ≤6000 截断训练集、特征用计数），与现行文献实现
#（响应 ef_dfi_g、协变量 ETP/OTD/FID、截尾只作用于协变量）不是同一套东西。
# 结论仍可作为历史记录读，但不可再用它推断现行行为。
