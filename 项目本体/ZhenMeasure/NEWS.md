...
## 1.2.0（未发布；版本号待定，本次为破坏性变更）

### ⚠️ 会改变表型结果

日级采食量校正（LMM）**整段重写为 Jiao et al. (2014, *J Anim Sci* 92:2377–2386) 的文献实现**（issue #5）。
默认配置下 `daily_feed_g` 的来源、模型形式与全部护栏都已改变，**ADFI / FCR 会明显移动**（ADG 不受影响，LMM 只作用于采食量）。
如果下游有已发布的结果依赖旧口径，请固定使用 `use_lmm_feed_correction = FALSE` 复现（见下"回归锁"）。
...
- **默认路径不再经过记录级物理纠正**（裁定 0）。原门控是「记录级纠正成功时跳过 LMM」，
  移除 stack 后若保留原门控，新 LMM 将永不运行，故门控收成
  `if (use_lmm_feed_correction) 跑 LMM else 只做出厂校验`。
  `use_record_feed_correction` 因此降级为「关掉 LMM 时的对照/消融键」。
  `.correct_feed_records()` 仍在计算（及内部 `feed_filtered` 列），但**其产物不再进入默认输出**。
...
### 实数据端到端回归（`use_lmm_feed_correction` 默认 TRUE vs V1.1.4 默认）
...
**回归锁（确认没有顺手改到记录级路径）**：`use_lmm_feed_correction = FALSE` 时，
...
### ⚠️ 注入式基准：新默认（L）的准确度低于被它旁路的记录级路径（A）

`测试/simulation_benchmark.R` 的变体矩阵随本次重写重定义为
**C0**（无校正）/ **A**（纯记录级物理纠正，V1.1.1 路径）/ **L**（文献 LMM，新默认）/
...
系统性估少。**这是文献方法与我们 flag 语义之间的真实落差，不是实现 bug**，也正是本次
"记录级规则 A 在默认路径上被旁路"（裁定 0）的量化版本：
...
**给下游的实用建议**：本次重写的可逆性已被逐值验证——
`use_lmm_feed_correction = FALSE` 精确复现 V1.1.4 的 A 路径（三设备 `max|Δ| = 0`）。
若业务更看重 ADFI 的逐日保真，这是现成的回退位；默认值是否翻转留待业务确认。
