...
步骤 2 删失模型 PoC（`测试/simulation_benchmark.R` `.em_censored_daily()`，未接入包管线）：噪声置零记录按右删失 Tobit 处理（对数尺度上界 speed_max×时长/60），lme4 混合模型 EM 迭代截尾条件期望（3-4 轮收敛）后复活。变体 G_cens 全档第一：accuracy 0.9648/0.9446/0.9140，20% 时总体 bias≈0（+0.04%）；negate 类恢复率 0.99–1.01、zero 类 0.965–0.986。待办：接入 Step5 + 其他设备数据泛化验证。
